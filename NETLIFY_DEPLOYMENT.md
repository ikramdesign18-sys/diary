...
This deploys only the Amanat Diary Groq proxy. The Express API in `artifacts/api-server` remains available for local development.
...
Do not add `GROQ_API_KEY` to the Expo app, `netlify.toml`, or any committed file.
...
5. Netlify reads these values from `netlify.toml`:
   - Build command: `pnpm --filter @workspace/api-server run typecheck && pnpm exec tsc -p netlify/tsconfig.json --noEmit`
